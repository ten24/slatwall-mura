	...
	<cffunction name="compile" access="public" returntype="string" output="false">
		...
		<cfscript>
			...
			try {
				...
				file = "";
				$fileDelete( tmpFile );
				$throw(  type    = 'Slatwall.org.cfstatic.util.LessCompiler.badLESS'
			...
			if ( not fileIsGlobal ) {
				for( i=1; i LTE ArrayLen(globals); i++ ){
					if ( not FileExists( globals[i] ) ) {
						$throw( "Slatwall.org.cfstatic.util.LessCompiler.missingGlobal", "Could not find LESS global, '#globals[i]#'" );
					}
