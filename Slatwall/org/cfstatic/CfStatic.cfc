...
<!--- private methods --->
	...
	<cffunction name="_packageDirectory" access="private" returntype="Slatwall.org.cfstatic.core.PackageCollection" output="false" hint="I take a directory and return a processed PackageCollection object (with stored metadata about the packages and files within it)">
		...
		<cfreturn CreateObject('component', 'Slatwall.org.cfstatic.core.PackageCollection').init(
	...
	<cffunction name="_loadCompilers" access="private" returntype="void" output="false" hint="I instantiate all the compilers used by cfstatic">
		...
		<cfscript>
			...
			_setYuiCompressor         ( CreateObject('component','Slatwall.org.cfstatic.util.YuiCompressor'       ).init( jlScope[jlScopeKey].yui                                      ) );
			_setLessCompiler          ( CreateObject('component','Slatwall.org.cfstatic.util.LessCompiler'        ).init( jlScope[jlScopeKey].less                                     ) );
			_setCoffeeScriptCompiler  ( CreateObject('component','Slatwall.org.cfstatic.util.CoffeeScriptCompiler').init( jlScope[jlScopeKey].coffee                                   ) );
	...
	<cffunction name="_loadJavaLoaders" access="private" output="false">
		<cfscript>
			...
			jarsForYui[1]    = ExpandPath('/Slatwall/org/cfstatic/lib/yuiCompressor/yuicompressor-2.4.7.jar');
			...
			jarsForCoffee[1] = ExpandPath('/Slatwall/org/cfstatic/lib/jcoffeescript/jcoffeescript-1.3.3.jar');
			...
			cfstaticJavaloaders.coffee = CreateObject('component','Slatwall.org.cfstatic.lib.javaloader.JavaLoader').init( jarsForCoffee );
    ...
    <cffunction name="_getDependenciesFromFile" access="private" returntype="struct" output="false">
    	...
    	<cfscript>
    		...
    		if ( Len(Trim( dependencyFile ) ) ) {
    			dependencies = CreateObject( 'component', 'Slatwall.org.cfstatic.util.DependencyFileParser' ).parse( dependencyFile, rootDir );
    		}
		...
		<cfscript>
			if ( not directoryExists( outputDirectory ) ) {
				try {
					...
					$throw(  type    = "Slatwall.org.cfstatic.CfStatic.badOutputDir"
